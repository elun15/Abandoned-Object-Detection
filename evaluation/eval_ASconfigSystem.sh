...
bin_path="./bin/"	 		#path where binaries are located (YOU CAN CHANGE)
...
FILESGT=$(find ${dataset_path} -name '*.txt') 
		...
		for fileRes in $full_res_path$i/*.xml; do
			...
			for fileGT in $FILESGT; do
				...
				if [[ $base == *"$baseGT"* ]]; then
					...
					echo ${exe_cmd} -g $fileGT -r $fileRes -raw $raw_file -o $out_file > ${out_log}
