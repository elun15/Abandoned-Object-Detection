...
void FrameDifferenceBGS::processfd(const cv::Mat &img_input, cv::Mat &img_output)
{
    if(img_input.empty())
        return;
    ...
    { imshow("Frame",img_input);
        imshow("Frame Difference!!", img_foreground);
        waitKey(1);}
...
{
    CvFileStorage* fs = cvOpenFileStorage("/home/vpu/Desktop/TFM_GitHub/TFM/lib/bgslibrary/config/FrameDifferenceBGS.xml", 0, CV_STORAGE_WRITE);
...
{
    CvFileStorage* fs = cvOpenFileStorage("/home/vpu/Desktop/TFM_GitHub/TFM/lib/bgslibrary/config/FrameDifferenceBGS.xml", 0, CV_STORAGE_READ);
