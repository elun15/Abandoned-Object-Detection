...
int StaticObjectClassifierPCC::checkObject(cv::Mat frame, cv::Mat bkgImage, cv::Mat staticObjMask, cv::Mat fgMask, ObjectBlob *object)
{
    ...
    if (largest_contour_index != -1)
    {
        ...
        double difference = valueBkg - valueImg; //score obtained
        //cout << "PCC dif : " << difference << endl;


        if (difference >= 0)
        {
            object->results->D_F = STATIC_OBJ_TYPE_STOLEN; //stolen decision (unkown by default)
        }
        else if (difference > -0.03 && difference < 0)
        {
            object->results->D_F = STATIC_OBJ_TYPE_UNKNOWN; //initial decision (unkown by default)
        }
        else // diff < -0.03
        {
            object->results->D_F = STATIC_OBJ_TYPE_ABANDONED; //abandoned decision (unkown by default)
